...
## 长报告写入协议（防 API 超时，最高优先级）

> 起因：一次性写入 600+ 行报告（≈25k 输出 token）会触发 `API Error: The operation timed out`。
> 实测分块 150–250 行可稳定通过。本协议对所有产出报告的 skill 生效。

### 分块写入

- **预判**：预计报告 >300 行，**必须**分块，不许尝试一次写完
- **单块上限 250 行**（约 6–8k 输出 token），宁小勿大
- **首块**用 `Write` 建立：标题 + 元信息 + 开头 1~2 节，末尾留锚点 `<!-- WIP -->`
- **后续每块**用 `Edit`：`old_string` = `<!-- WIP -->`，`new_string` = 本块内容 + `<!-- WIP -->`
- **末块**的 `new_string` **不带**锚点，自然收尾
- 每块写完**不要回读验证** —— `Edit` 失败会直接报错，harness 已跟踪文件状态

### 中断恢复（关键）

- 任何时候发现目标文件末尾有 `<!-- WIP -->`，说明上次写到该处 —— **从锚点继续追加，禁止重头重写**
- 恢复前先 `wc -l` 并读文件末尾 20 行确认断点，再续写

### 完工自查

```bash
grep -c "WIP" <报告路径>   # 必须为 0，否则说明没写完
wc -l <报告路径>
```

### 其他大输出场景（同样会超时）

- **不要**在一次对话响应里输出整份长表格或整章分析 —— 拆成多轮
- **大工具输出必须过滤再读**：如 `tools/valuation_percentile.py` 单次输出可达 ~98KB，
  应 `grep -E "历史百分位|所处区间|当前值|median"` 提取摘要，不全量读入上下文
- **TuShare 批量取数**：调用间加 `sleep 2`~`sleep 3` 规避 HTTP 429
  （`未开启 gzip，当前限制线程`），失败重试而非放弃
