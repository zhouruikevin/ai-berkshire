...
- `skills/*.md` 是**唯一 canonical 源**。本目录下的 `.qoder/skills/*/SKILL.md` 由脚本生成，**不要直接手改**。
- 改完 `skills/*.md` 后，运行 `python3 scripts/sync-qoder-skills.py` 重新生成 Qoder 技能；同时运行 `sync-codex-skills.py`、`sync-codex-prompts.py` 保持三平台一致。
- 校验是否同步：`python3 scripts/sync-qoder-skills.py --check`（退出码非零表示已脱节）。
