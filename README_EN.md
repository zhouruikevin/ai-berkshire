...
**AI Berkshire** is a collection of investment research skills built for **Qoder** as the primary environment, and also compatible with Claude Code and Codex. It systematizes the methodologies of four value investing masters — Buffett, Munger, Duan Yongping, and Li Lu — and delivers professional-grade research through AI Agents.
...
> **Qoder is recommended.** This repo ships `.qoder/` (rules + 18 project-level skills), so opening the repo in Qoder auto-loads everything with zero setup. Claude Code / Codex are supported as compatibility entry points.

### 1. Install an AI Client

This repository keeps one canonical workflow and provides Qoder skills, Claude Code commands, and Codex skills. Install the client you plan to use.

For Qoder users (recommended):

Download and install Qoder IDE from [qoder.com](https://qoder.com), then open this repository with it — `.qoder/skills` load automatically as project-level skills and `.qoder/rules` are injected as research guidelines.
...
# Just open the directory in Qoder — project-level skills load automatically, no install needed.
# To use them across all projects, install to the user-level ~/.qoder/skills:
cd ai-berkshire
...
The repository treats `skills/*.md` as the **single canonical source**; every other entry point is generated from it: `.qoder/skills/*/SKILL.md` are Qoder project-level skills generated by `scripts/sync-qoder-skills.py`; `codex-skills/*/SKILL.md` are Codex skill packages generated by `scripts/sync-codex-skills.py`; `codex-prompts/*.md` are an optional Codex slash-prompt compatibility layer. After editing `skills/*.md`, run these three sync scripts to keep all platforms in sync.

### 3. Use

In Qoder, type `/` to see all 18 skills and pick one, or describe your task in natural language — Qoder auto-matches the right skill by its description. Slash invocation matches the Claude Code examples below (e.g. `/investment-research Tencent`).
